...
# ── Top 7 characters by line count ────────────────────────────────────────────
...
# ── Asymmetric chord matrix: lines spoken in shared scenes ────────────────────
# matrix[i][j] = lines character i speaks in scenes where j also appears.
# Weight by 1/(k-1) so that a scene with k top-7 chars doesn't inflate totals.
# Diagonal  = lines spoken in scenes with NO other top-7 character present.
# Result: arc total = actual total lines spoken by that character.
chord_matrix_lines <- {
  sl <- hamlet %>%
    filter(character %in% main_characters) %>%
  ...
  mat <- matrix(0.0,
    nrow = length(main_characters), ncol = length(main_characters),
    dimnames = list(main_characters, main_characters))

  # Off-diagonal: pairwise weighted contributions
  pair_contribs <- sl %>%
    filter(n_top7 >= 2) %>%
    inner_join(
      sl %>% filter(n_top7 >= 2) %>% select(act, scene, char_j = character),
      by = c("act", "scene")
    ) %>%
    filter(character != char_j) %>%
    mutate(contrib = lines / (n_top7 - 1)) %>%
    group_by(character, char_j) %>%
    summarise(total = sum(contrib), .groups = "drop")

  for (k in seq_len(nrow(pair_contribs)))
    mat[pair_contribs$character[k], pair_contribs$char_j[k]] <- pair_contribs$total[k]

  # Diagonal: solo scenes (no other top-7 character present)
  solo_contribs <- sl %>%
    filter(n_top7 == 1) %>%
    group_by(character) %>%
    summarise(solo = sum(lines), .groups = "drop")

  for (k in seq_len(nrow(solo_contribs)))
    mat[solo_contribs$character[k], solo_contribs$character[k]] <- solo_contribs$solo[k]
...
draw_chord <- function() {
  ...
  # Ribbon color = source character, semi-transparent; NA diagonal = no self-loop ribbon
  n       <- length(chars)
  col_mat <- matrix(NA_character_, n, n, dimnames = list(chars, chars))
  for (i in seq_len(n))
    for (j in seq_len(n))
      if (i != j) col_mat[i, j] <- adjustcolor(cols[i], alpha.f = 0.45)

  par(bg = "#F5E6C8", mar = c(2, 2, 4, 2), family = "Garamond")
  circos.clear()
  circos.par(
    start.degree            = 90,
    gap.degree              = 3,
  ...
  chordDiagram(
    ...
    link.border       = adjustcolor("#2C1810", alpha.f = 0.20),
    link.lwd          = 0.4,
    self.link         = 1
  )

  # Character labels — clockwise, colored, bold
  circos.trackPlotRegion(track.index = 1, panel.fun = function(x, y) {
    nm   <- get.cell.meta.data("sector.index")
    xlim <- get.cell.meta.data("xlim")
    ylim <- get.cell.meta.data("ylim")
    circos.text(
      mean(xlim), ylim[1] + 0.1, nm,
      facing     = "clockwise",
      niceFacing = TRUE,
      adj        = c(0, 0.5),
      col        = cols[nm],
  ...
  mtext(
    "Arc length = total lines spoken by character",
    side = 3, line = 0.5,
